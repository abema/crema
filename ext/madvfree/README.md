...
Linux-only, best-effort `crema.CacheProvider[[]byte]` backed by one anonymous
`mmap` arena.

Idle extents are marked `MADV_FREE`. Linux may discard them without swap I/O
under memory pressure, so `Get` can return a cache miss before TTL expiry.
Pages returned to the internal allocator use `MADV_DONTNEED`. Returned values
are always copied into Go-managed memory. The arena uses `MAP_NORESERVE`,
allowing its virtual size to exceed the immediately available physical memory
without reserving swap for every page.
...
use extents. A slab is marked `MADV_FREE` only when it has no active readers.
...
64 index shards, `MADV_NOHUGEPAGE`, and `MADV_DONTDUMP`. `MAP_NORESERVE` means
the virtual arena does not eagerly reserve 64 GiB of physical memory or swap.
Set `CapacityBytes`, `SizeClasses`, or `ShardCount` to override their defaults.
Set `EnableHugePages` or `IncludeInCoreDump` to opt out of the corresponding
default advice.

`MAP_NORESERVE` does not guarantee that the default 64 GiB mapping will
succeed. A strict overcommit policy, a low `RLIMIT_AS`, or an equivalent
container limit can make `NewProvider` fail with an `mmap` `ENOMEM`. Set
`CapacityBytes` explicitly below that environment's virtual-address-space
limit.

The provider supports 64-bit Linux. Other platforms return
`madvfree.ErrUnsupported`. `NewProvider` also probes `MADV_FREE` against a
real anonymous private mapping and returns an error wrapping
`madvfree.ErrUnsupported` when the running kernel cannot use it.

After initialization, `MADV_FREE` and `MADV_DONTNEED` failures are soft
failures. Cache indexing, generations, and arena accounting continue normally,
but RSS may remain higher because the kernel did not accept the reclaim hint.
`Stats.MadvFreeErrors` and `Stats.MadvDontNeedErrors` expose these failures.

## Capacity and eviction

Kernel reclaim and allocator capacity are separate. Reclaiming a `MADV_FREE`
page reduces physical memory pressure, but does not by itself return the
...
logical arena and receive `madvfree.ErrCapacity`, even after Linux has reclaimed
physical pages and reduced RSS. Use TTLs for time-bounded entries, or call
`Trim` before retrying when the application requires space immediately.
...
whether a `MADV_FREE` page was reclaimed requires write-touching it; doing that
to a page that has not yet been reclaimed cancels its lazy-free state and pins
it again.

## Comparison benchmarks

Linux benchmarks compare this provider with `ext/ristretto` at 64 B, 4 KiB,
6 KiB, and 64 KiB:
...
The opt-in Linux integration test measures the effective available memory at
runtime. It prefers the cgroup v2 or v1 memory limit and falls back to
`MemAvailable`. It writes 1.5 times that amount into the arena, then verifies
that Linux reclaimed at least one `MADV_FREE` extent and that every cache hit
...
The default safety ceiling is 4 GiB of writes. A larger environment is skipped
unless the ceiling is explicitly raised:
